<x-app-layout>
    @section('title', 'Shop by Category | Prokiti Sudha')
    ...
    <x-slot name="main">
        ...
        <section class="relative pt-xl pb-lg px-gutter overflow-hidden">
            ...
            <div class="max-w-container-max mx-auto text-center relative z-10">
                <span class="font-label-md text-label-md text-tertiary uppercase tracking-widest mb-4 block">Our Collections</span>
                <h1 class="font-display-lg text-display-lg-mobile md:text-display-lg text-primary mb-6">Shop by Category</h1>
        ...
        <section class="py-lg px-gutter max-w-container-max mx-auto">
            <div class="flex items-end justify-between mb-md">
                ...
                <div class="flex gap-xs">
                    <button class="w-10 h-10 rounded-full border border-outline-variant flex items-center justify-center text-on-surface-variant hover:border-tertiary hover:text-tertiary transition-all" id="trending-prev">
                        <span class="material-symbols-outlined">chevron_left</span>
                    </button>
                    <button class="w-10 h-10 rounded-full border border-outline-variant flex items-center justify-center text-on-surface-variant hover:border-tertiary hover:text-tertiary transition-all" id="trending-next">
            ...
            <div class="flex gap-md overflow-x-auto pb-6 custom-scrollbar snap-x" id="trending-carousel">
                ...
                <div class="flex-none w-64 snap-start">
                    <a href="{{ route('public.products', ['category' => $featured->slug]) }}" class="group cursor-pointer block">
                        ...
                        <p class="text-caption text-on-surface-variant uppercase tracking-tighter">Featured Collection</p>
                    </a>
                </div>
                @endforeach
            </div>
        ...
        <section class="py-xl bg-surface-container-low px-gutter">
            <div class="max-w-container-max mx-auto">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-md md:gap-lg">
                    ...
                    <div class="group bg-surface rounded-[24px] overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 border border-surface-variant flex flex-col h-full opacity-0 translate-y-10 reveal-item">
                        <div class="relative h-64 overflow-hidden">
                            <div class="w-full h-full bg-cover bg-center transition-transform duration-700 group-hover:scale-110"
                                style="background-image: url('{{ $category->image ? Storage::url($category->image) : 'https://placehold.co/600x400?text=No+Image' }}')">
                            </div>
                            <div class="absolute top-4 left-4 bg-surface/90 backdrop-blur-md w-12 h-12 rounded-full flex items-center justify-center text-primary">
                        ...
                        <div class="p-md flex flex-col flex-grow">
                            <div class="flex justify-between items-start mb-2">
                                ...
                                <span class="text-caption text-tertiary-container font-bold bg-tertiary-fixed px-2 py-0.5 rounded-full">
                                    {{ $category->products_count ?? 0 }} Products
                                </span>
                            </div>
                            @if($category->description)
                            <p class="text-body-md text-on-surface-variant mb-6 line-clamp-2">{{ $category->description }}</p>
                            @else
                            <p class="text-body-md text-on-surface-variant mb-6 line-clamp-2">Discover our premium {{ strtolower($category->name) }} collection.</p>
                            @endif
        ...
        <section class="py-xl px-gutter">
            <div class="max-w-container-max mx-auto relative rounded-[32px] overflow-hidden min-h-[500px] flex items-center">
                ...
                <div class="relative z-10 px-md md:px-xl py-lg max-w-2xl text-white">
                    <span class="font-label-md text-label-md uppercase tracking-[0.2em] mb-4 block text-secondary-fixed">Nature's Wisdom</span>
                    <h2 class="font-display-lg text-headline-lg md:text-display-lg mb-6 leading-tight">The Ancient Secret of Longevity</h2>
                    <p class="font-body-lg text-body-lg mb-8 opacity-90">Discover our signature collection featuring heirloom seeds and rare herbal formulations passed down through generations of natural practitioners.</p>
                    <a href="{{ route('public.products') }}" class="inline-flex bg-primary-container hover:bg-primary text-white px-8 py-4 rounded-xl font-label-md uppercase tracking-wider transition-all duration-300 items-center gap-3">
        ...
        <section class="bg-surface-container py-xl px-gutter relative overflow-hidden">
            <div class="max-w-3xl mx-auto text-center relative z-10">
                ...
                <form class="flex flex-col md:flex-row gap-4 max-w-lg mx-auto">
                    <input class="flex-grow bg-white border-0 border-b-2 border-outline-variant focus:border-tertiary focus:ring-0 px-4 py-4 text-body-md rounded-lg shadow-sm"
                        placeholder="Your Email Address" required="" type="email" />
                    <button class="bg-primary text-white font-label-md uppercase tracking-widest px-8 py-4 rounded-xl hover:bg-primary-container transition-all shadow-md active:scale-95"
                        type="submit">
                        Subscribe
                    </button>
                </form>
                <p class="text-caption text-on-surface-variant mt-4 opacity-70">By subscribing, you agree to our Privacy Policy and Terms of Service.</p>
            </div>
